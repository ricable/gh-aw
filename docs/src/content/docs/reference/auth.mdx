...
- **GitHub Projects v2 safe-outputs (writing)** – Add [`GH_AW_PROJECT_GITHUB_TOKEN`](#gh_aw_project_github_token)
...
If using GitHub Projects v2 **safe-outputs** (writing to projects), you need to configure this GitHub Actions secret with a Personal Access Token (PAT) that has the appropriate scopes and permissions for Projects access.

This is a specialized token for [GitHub Projects](https://docs.github.com/en/issues/planning-and-tracking-with-projects/learning-about-projects/about-projects) **write operations via safe-outputs**, required because the default `GITHUB_TOKEN` cannot access the Projects API.

**When to use**:

- **Writing to projects** via safe-outputs (`update-project`, `create-project`, `create-project-status-update`).
- When you want to isolate Projects permissions from other workflow operations.

> [!IMPORTANT]
> **Reading vs. Writing to Projects**
>
> This token is **only** used by safe-outputs for **writing** to projects. If your workflow also needs to **read** project data using the GitHub tool (via `toolsets: [projects]`), you must separately configure `tools.github.github-token`. See [GitHub Tools (`github:`)](/gh-aw/reference/tools/#github-tools-github) for configuration details.
>
> ```yaml wrap
> # ✅ Correct: Configure token for both reading (GitHub tool) and writing (safe-outputs)
> tools:
>   github:
...
**Token precedence for safe-outputs (writing to projects)**:

per-safe-output `github-token` → `GH_AW_PROJECT_GITHUB_TOKEN`

> [!NOTE]
> Unlike other tokens, `GH_AW_PROJECT_GITHUB_TOKEN` does **not** fall back to `GITHUB_TOKEN` because `GITHUB_TOKEN` cannot access GitHub Projects v2 API.

**Token precedence for GitHub tools (reading projects)**:

`tools.github.github-token` → `GH_AW_GITHUB_MCP_SERVER_TOKEN` → `GH_AW_GITHUB_TOKEN` → `GITHUB_TOKEN`

> [!WARNING]
> `GH_AW_PROJECT_GITHUB_TOKEN` is **not** automatically used by the GitHub tool. If you want to read projects using `toolsets: [projects]`, you must explicitly set `tools.github.github-token`.

**Example configuration**:

```yaml wrap
---
# Option 1: Writing only - Use GH_AW_PROJECT_GITHUB_TOKEN secret for safe-outputs
# Just create the secret - no additional workflow config needed for writing
safe-outputs:
  update-project:
    project: https://github.com/orgs/myorg/projects/123
---

# Option 2: Reading and writing - Configure both GitHub tools and safe-outputs
# Required when your agent needs to read project data (e.g., check existing items)
tools:
  github:
    toolsets: [default, projects]
    github-token: ${{ secrets.GH_AW_PROJECT_GITHUB_TOKEN }}  # For reading
safe-outputs:
  update-project:
    github-token: ${{ secrets.GH_AW_PROJECT_GITHUB_TOKEN }}  # For writing
```

**Notes**:

- **Reading vs. writing**: `GH_AW_PROJECT_GITHUB_TOKEN` only applies to safe-outputs (writing). To read projects via the GitHub tool, configure `tools.github.github-token` separately.
- For workflows that both read and write projects, configure the same token in both `tools.github.github-token` and `safe-outputs.*.github-token`.
